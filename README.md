...
2. **Install Dependencies:**
   ...
   Install all required packages and local modules (serl_launcher, franka_sim, UR5e_sim, serl_robot_infra) using the requirements file.
...
1. Change directory to target folder
    ...
    ```bash
    cd examples/experiments/stack_cube_sim
    ```
...
4. Start human-in-loop RL training. Before runnning the following two bash file, you need to check and update `run_actor.sh` and `run_learner.sh`. For example the *exp_name* must be one of the folder name in experiments, the *demo_path* need to be the absolute path of your `.pkl` file(not relative path).
    ...
    export XLA_PYTHON_CLIENT_MEM_FRACTION=.40  # Limit GPU memory usage to 40%
    bash run_learner.sh

    export XLA_PYTHON_CLIENT_MEM_FRACTION=.20  # Limit GPU memory usage to 20%
